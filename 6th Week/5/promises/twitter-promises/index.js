...
const promisedTweets = util.promisify(getTweets);
...
app.get('/data.json', (req, res) => {
    ...
    // getToken with promises
    promisedToken()
        .then((bearerToken) => {
            promisedTweets(bearerToken)
                .then((tweets) => {
                    const filteredTweets = filterTweets(tweets);
                    res.json(filteredTweets);
                })
                .catch((err) => {
                    console.log('error in promisedTweets :>> ', err);
                });
        })
        .catch((err) => {
            console.log('error in promisedToken :>> ', err);
        });
});
